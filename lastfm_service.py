...
class LastFmService:
    ...
    def fetch_tracks_by_tag(self, tag, limit=10):
        ...
        try:
            ...
            raise Exception(
                f"Error while connecting to Last.fm servers: {error}")

    def generate_tracklist(self, tags, track_count):
        print(
            f"Generating tracklist for {track_count} tracks using tags: {tags}")
        ...
        for tag in tags:
            ...
            try:
                tracks = self.fetch_tracks_by_tag(
                    tag, limit=fetch_limit_per_tag)
